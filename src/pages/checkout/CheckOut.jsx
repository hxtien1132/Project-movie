...
import { datVeAction, layChiTietPhongVeAction } from '../../redux/actions/QuanLiDatVeAction';
import style from './CheckOut.module.css'
import './CheckOut.css';
import _ from 'lodash'
import { CheckOutlined, CloseOutlined, UserOutlined, SmileOutlined, HomeOutlined } from '@ant-design/icons'
import { DAT_VE } from "../../redux/types/QuanLiDatVeTypes.js";
...
function CheckOut(props) {
  ...
  const { chiTietPhongVe, danhSachGheDangDat, danhSachGheKhachDat } = useSelector(state => state.QuanLiDatVeReducer);
    ...
    <div className=' min-h-screen' style={{ minHeight: '100vh' }}>
      <div className="grid grid-cols-12">
        ...
        <div className="col-span-3">
          ...
          <hr />
          <div className="mb-5 flex flex-col justify-end items-center">
            <div
              onClick={() => {
                ...
                distpatch(datVeAction(thongTinDatVe));
              }}
...
export default function CheckOutTabs(props) {
  ...
  const operations = <Fragment>
    {!_.isEmpty(userLogin) ? <Fragment> <button onClick={() => {
      navigate('/profile')
    }}> <div style={{ width: 50, height: 50, display: 'flex', justifyContent: 'center', alignItems: 'center' }} className="text-2xl ml-5 rounded-full bg-red-200 mr-5">{userLogin.taiKhoan.substr(0, 1)}</div>Hello ! {userLogin.taiKhoan}</button> <button onClick={() => {
      localStorage.removeItem(USER_LOGIN);
      localStorage.removeItem(TOKEN);
      navigate("/")
      window.location.reload();
    }} className="text-blue-800 mr-3 btn btn-success">Đăng xuất</button> </Fragment> : ''}
